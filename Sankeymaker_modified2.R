...
countryRegions <- lapply(regions, function(z){
  ...
  regions_out <- unique(flows$source[flows$source!=z])
  regions_out <- regions_out[grep(pattern = " >", x = regions_out, fixed = FALSE)]
  regions_out <- c(regions_out, cNodes[grep(pattern = "-GB", x = cNodes, fixed = FALSE)])
  cNodes <- c(regions_out, setdiff(cNodes, regions_out)); rm(regions_out)
...
c <- sankeyNetwork(Links = subsankey$sankey$links, Nodes = subsankey$sankey$nodes, Source = 'source',
              Target = 'target', Value = 'value', NodeID = 'name',
              units = 'samples', fontSize = 12, nodeWidth = 30)
saveNetwork(c,paste("sankey_asia2regions.html",sep=""), selfcontained = TRUE)
...
countryRegions2 <- lapply(regions2, function(z){
  ...
  cNodes <- c(cFlows$source, cFlows$target) %>% unique %>% sort
  regions_out <- unique(flows$target[flows$target!=z])
  regions_out <- regions_out[grep(pattern = "^> ", x = regions_out, fixed = FALSE)]
  regions_out <- c(regions_out, cNodes[grep(pattern = "-GB", x = cNodes, fixed = FALSE)])
  cNodes <- c(regions_out, setdiff(cNodes, regions_out)); rm(regions_out)
...
# sankeyNetwork(Links = subsankey$sankey$links, Nodes = subsankey$sankey$nodes, Source = 'source',
#               Target = 'target', Value = 'value', NodeID = 'name',
#               units = 'samples', fontSize = 12, nodeWidth = 30)
# saveNetwork(c,paste("sankey_africa2regions.html",sep=""), selfcontained = TRUE)
...
sink('sankey_draft_subSankey_corrected4.json') # redirect console output to a file
toJSON(cJson, pretty=FALSE)
sink()
